...
class ModelsellerdashboardSale extends Model
{
    ...
    {
        $query = $this->db->query('SELECT COUNT(*) AS total, SUM(o.total) AS amount, c.iso_code_2 FROM `'.DB_PREFIX.'product_to_seller` cp
		LEFT JOIN	`'.DB_PREFIX.'order_product` op ON (cp.product_id = op.product_id)
    ...
    public function getTotalOrders($data = array())
    {
        ...
        switch ($group) {
            case 'day';
                $sql = 'SELECT COUNT(DISTINCT YEAR(date_added), MONTH(date_added), DAY(date_added)) AS total FROM `'.DB_PREFIX.'product_to_seller` cp
		LEFT JOIN	`'.DB_PREFIX.'order_product` op ON (cp.product_id = op.product_id)
		LEFT JOIN  `'.DB_PREFIX.'order` o  ON (o.order_id = op.order_id) ';
                break;
            default:
            case 'week':
                $sql = 'SELECT COUNT(DISTINCT YEAR(date_added), WEEK(date_added)) AS total FROM `'.DB_PREFIX.'product_to_seller` cp
		LEFT JOIN	`'.DB_PREFIX.'order_product` op ON (cp.product_id = op.product_id)
		LEFT JOIN  `'.DB_PREFIX.'order` o  ON (o.order_id = op.order_id) ';
                break;
            case 'month':
                $sql = 'SELECT COUNT(DISTINCT YEAR(date_added), MONTH(date_added)) AS total FROM `'.DB_PREFIX.'product_to_seller` cp
		LEFT JOIN	`'.DB_PREFIX.'order_product` op ON (cp.product_id = op.product_id)
		LEFT JOIN  `'.DB_PREFIX.'order` o  ON (o.order_id = op.order_id) ';
                break;
            case 'year':
                $sql = 'SELECT COUNT(DISTINCT YEAR(date_added)) AS total FROM `'.DB_PREFIX.'product_to_seller` cp
		LEFT JOIN	`'.DB_PREFIX.'order_product` op ON (cp.product_id = op.product_id)
		LEFT JOIN  `'.DB_PREFIX.'order` o  ON (o.order_id = op.order_id) ';
                break;
        }
    ...
    {
        $sql = 'SELECT MIN(o.date_added) AS date_start, MAX(o.date_added) AS date_end, ot.title, SUM(ot.value) AS total, COUNT(o.order_id) AS `orders` FROM `'.DB_PREFIX.'product_to_seller` cp
		LEFT JOIN	`'.DB_PREFIX.'order_product` op ON (cp.product_id = op.product_id)
    ...
    public function getTotalTaxes($data = array())
    {
        ...
        switch ($group) {
            case 'day';
                $sql = 'SELECT COUNT(DISTINCT YEAR(o.date_added), MONTH(o.date_added), DAY(o.date_added), ot.title) AS total FROM `'.DB_PREFIX.'product_to_seller` cp
		LEFT JOIN	`'.DB_PREFIX.'order_product` op ON (cp.product_id = op.product_id)
		LEFT JOIN  `'.DB_PREFIX.'order` o  ON (o.order_id = op.order_id)';
                break;
            default:
            case 'week':
                $sql = 'SELECT COUNT(DISTINCT YEAR(o.date_added), WEEK(o.date_added), ot.title) AS total FROM `'.DB_PREFIX.'product_to_seller` cp
		LEFT JOIN	`'.DB_PREFIX.'order_product` op ON (cp.product_id = op.product_id)
		LEFT JOIN  `'.DB_PREFIX.'order` o  ON (o.order_id = op.order_id)';
                break;
            case 'month':
                $sql = 'SELECT COUNT(DISTINCT YEAR(o.date_added), MONTH(o.date_added), ot.title) AS total FROM `'.DB_PREFIX.'product_to_seller` cp
		LEFT JOIN	`'.DB_PREFIX.'order_product` op ON (cp.product_id = op.product_id)
		LEFT JOIN  `'.DB_PREFIX.'order` o  ON (o.order_id = op.order_id)';
                break;
            case 'year':
                $sql = 'SELECT COUNT(DISTINCT YEAR(o.date_added), ot.title) AS total FROM `'.DB_PREFIX.'product_to_seller` cp
		LEFT JOIN	`'.DB_PREFIX.'order_product` op ON (cp.product_id = op.product_id)
		LEFT JOIN  `'.DB_PREFIX.'order` o  ON (o.order_id = op.order_id)';
                break;
        }
