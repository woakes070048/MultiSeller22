	...
	public function addOrder($data) {
		...
		if (isset($data['totals'])) {
			foreach ($data['totals'] as $total) {
				if(isset($total['seller_id'])){
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_total SET order_id = '" . (int)$order_id . "', code = '" . $this->db->escape($total['code']) . "', title = '" . $this->db->escape($total['title']) . "', `value` = '" . (float)$total['value'] . "',`seller_id` = '" . (int)$total['seller_id'] . "',`product_id` = '" . (int)$total['product_id'] . "', sort_order = '" . (int)$total['sort_order'] . "'");
			}else{
		...
		// Totals
		if (isset($data['seller_shippings'])) {
			foreach ($data['seller_shippings'] as $seller_shipping) {
				$this->db->query("UPDATE " . DB_PREFIX . "order_product SET shipping_method = '" . $seller_shipping['shipping_method'] . "',shipping_code = '" . $seller_shipping['shipping_code'] . "' WHERE product_id = '" . $seller_shipping['product_id'] . "' AND order_id = '" . (int)$order_id . "'");
			}
		}
